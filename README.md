...
3. AI 讀取你的 repo、理解你的專案慣例（透過 `.legate/CLAUDE.md`）、處理任務
...
1. 在你的 repo 根目錄建立 `.legate/`：

```
.legate/
├── config.yaml     # 基本設定
└── CLAUDE.md       # 告訴 AI 你的專案規矩
```
...
3. 透過支援的 Channel 發送任務（目前：在 GitHub 開 Issue 並加上 `legate` label）

4. 收到 PR 後 review，留 comment 可以要求修改，滿意就 merge
